defmodule Distributed.ExUnit.Runner do
  @moduledoc false
  ...
  def run_module(config, module) do
    ...
    me = self()
    IO.inspect("IN HERE")
    Node.spawn(:"drone3@devon-XPS-13-9370", fn ->
      result = run_module(config, test_module, to_run_tests)
  ...
  def run_module(config, test_module, tests) do
  ...
  defp run_setup_all(%ExUnit.TestModule{name: module} = test_module, parent_pid) do
    ...
    spawn_monitor(fn ->
      Distributed.ExUnit.OnExitHandler.register(self())
  ...
  defp exec_on_exit(test_or_case, pid, timeout) do
    case Distributed.ExUnit.OnExitHandler.run(pid, timeout) do
      :ok ->
        test_or_case
