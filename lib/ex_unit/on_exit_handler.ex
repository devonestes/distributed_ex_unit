defmodule Distributed.ExUnit.OnExitHandler do
  @moduledoc false
  ...
  def start_link(_opts) do
    :rpc.call(:"leader@devon-XPS-13-9370", Agent, :start_link, [fn -> :ets.new(@name, @ets_opts) end, [name: @name]])
    |> IO.inspect()
  end

  @spec register(pid) :: :ok
  def register(pid) when is_pid(pid) do
    :rpc.call(:"leader@devon-XPS-13-9370", :ets, :insert, [@name, {pid, nil, []}])
    :ok
  end
  ...
  def add(pid, name_or_ref, callback) when is_pid(pid) and is_function(callback, 0) do
    try do
      :rpc.call(:"leader@devon-XPS-13-9370", :ets, :lookup_element, [@name, pid, @on_exit])
    rescue
      _ -> :error
    else
      entries ->
  ...
  def run(pid, timeout) when is_pid(pid) do
    [{^pid, sup, callbacks}] = :rpc.call(:"leader@devon-XPS-13-9370", :ets, :insert, [@name, {pid, nil, []}])
